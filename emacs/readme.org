...
Gnus first so Gnus should be configured before loading Org, see
[[*\[\[https://www.gnus.org/\]\[Gnus\]\]]]) =apropos-value=,
...
(use-package htmlize
  ...
  (let* ((fc-match "fc-match -f '%{file}' 'Iosevka Term'")
    ...
    ;; This is very likely to be a suboptimal choice for web hosting.
...
(use-package eglot
  ...
  (setq eglot-sync-connect nil ; Asynchronously establish the connection.
        eldoc-display-functions '(eldoc-display-in-buffer))) ; Eliminate all noise in minibuffer.
#+end_src
...
**** [[https://www.gnus.org/][Gnus]]

A bunch of settings to make the default behavior less surprising.

#+begin_src emacs-lisp
(use-package gnus ; Use the embedded Gnus.
  :defer t
  :init
  (setq
   ;; Defaults to ~/News, must be done before loading Gnus.
   ;; Beware, this is copy/pasted in init.el: when asked to tangle Org will
   ;; happen to load Gnus and sadly this variable is used as the basis of many
   ;; others so it must also be set before loading Org.
   gnus-home-directory (expand-file-name "gnus" user-emacs-directory)
   ;; Ensure this configuration is idempotent. My local configuration might
   ;; change this, for example after `my--org-tangle'.
   gnus-init-file nil)
  (make-directory gnus-home-directory t) ; Gnus will complain otherwise.
  :config
  (setq
   ;; I don't use another newsreader so don't maintain ~/.newsrc
   ;; https://www.gnu.org/software/emacs/manual/html_node/gnus/Startup-Files.html
   gnus-read-newsrc-file nil
   gnus-save-newsrc-file nil
   ;; Don't auto-save, sometimes useful to see how Gnus would store changes.
   gnus-use-dribble-file nil
   ;; This must be set but defaults to an NNTP backend.
   ;; https://www.gnu.org/software/emacs/manual/html_node/gnus/The-Empty-Backend.html
   ;; Ideally all sources are added to `gnus-secondary-select-methods' so
   ;; prefixing stays consistant.
   gnus-select-method '(nnnil "")
   ;; No 'nnfolder+archive:sent...' group.
   ;; https://www.gnu.org/software/emacs/manual/html_node/gnus/Archived-Messages.html
   ;; Depending on the source, this might be achieved with the Gcc header in
   ;; `gnus-parameters' or `gnus-posting-styles' (otherwise sent emails might not
