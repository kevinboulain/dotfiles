...
Simpler the mode-line format (I'd prefer moving this information to the
minibuffer but it has a lot of edge cases that are difficult or impossible
to handle):
#+begin_src emacs-lisp
...
The below configuration even allows to call Emacs like this:
#+begin_src shell
