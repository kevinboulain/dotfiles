{
  ...
  outputs =
    ...
    }:
    {
      nixosConfigurations =
        let
          # Sadly, it doesn't look like there's an easy way to get the public
          # key encrypted with sops-nix: AuthorizedKeysFile would have to be
          # tweaked and that wouldn't solve interactive decryption in the initrd
          # (the key defaults to
          ...
          specialArgs = rec {
            ...
            myHostsLib = import ./hosts/lib {
              inherit (nixpkgs) lib;
              inherit mySystemDirectory;
            };
          };
        in
        {
              ...
              ++ [
                {
                  ...
                  boot.kernelParams = [
                    # Run 'btrfs inspect-internal map-swapfile path/to/swap' to
                    # find out. A full rebalance might break it:
